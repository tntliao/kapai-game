<template>
	<view class="app">
		<view class="app_bg">
			<!-- 顶部 -->
			<topnav :title="$t('list.challenge')" urlline="bosslevel"></topnav>
				...
				<view class="list_info" v-for="(item,i) in tzlist">
					...
					<view class="info_right">
						<view class="info_data">
							...
							<view class="data_num" v-if="item.taId==1">{{$t('list.jungle')}}</view>
							<view class="data_num" v-if="item.taId==2">{{$t('list.ice')}}</view>
							<view class="data_num" v-if="item.taId==3">{{$t('list.ajatar')}}</view>
						</view>
						<view class="info_data">
						...
						<view class="info_data">
							<view class="data_name">{{$t('list.get')}}：</view>
							<!-- <view class="data_num time_box">1000LOO</view> -->
							<view class="data_num time_box" v-if="item.scode==0">{{$t('list.no')}}</view>
							<view class="data_num time_box" v-if="item.scode==801">{{$t('all.s801')}} x{{item.num}}</view>
							<view class="data_num time_box" v-if="item.scode==802">{{$t('all.s802')}} x{{item.num}}</view>
							<view class="data_num time_box" v-if="item.scode==803">{{$t('all.s803')}} x{{item.num}}</view>
							<view class="data_num time_box" v-if="item.scode==804">{{symbol}} x{{item.num}}</view>
							<view class="data_num time_box" v-if="item.scode==805">{{$t('all.s805')}} x{{item.num}}</view>
							<view class="data_num time_box" v-if="item.scode==806">{{$t('all.s806')}} x{{item.num}}</view>
							<view class="data_num time_box" v-if="item.scode==807">{{$t('all.s807')}} x{{item.num}}</view>
							<view class="data_num time_box" v-if="item.scode==808">{{$t('all.s808')}} x{{item.num}}</view>
						</view>
					</view>
				</view>
				<!-- 加载更多动画 -->

					...
					<view class="table-list">
						<table>
							...
							<tr v-for="(item,index) in zt2arr" :key="index"
								@click="jup(`/pages/zhitui/three/three`,item.userId,type)">
								<td>{{item.addtime}}</td>
								<td>{{item.userId}}</td>
			...
			getinfor() {
				if (this.type == 0) {
					...
					function czSuc(res) {
						...
						console.log( res.data.czArr,'走了')
					}
					that.Utils.tools.post(url, data, that, czSuc);
				}

<template>
	<view class="app">
		<view class="app_bg">
			<!-- 顶部 -->
			<topnav :title="$t('list.pmlist')" urlline="backpack"></topnav>
			...
			<scroll-view class="cs_list" scroll-y="true" @scrolltolower="dddd()">
				<view class="list_info" v-for="(item,i) in pmlist">
					<view class="card_content">
						...
						<!-- 状态 -->
						<view class="list_type" v-if="item.status==0">{{$t('list.pming')}}</view>
						<view class="list_type" v-if="item.status==1">{{$t('list.closed')}}</view>
						<view class="list_type" v-if="item.status==-1">{{$t('list.xiajia')}}</view>
						<view class="list_type" v-if="item.status==-2">{{$t('list.beat')}}</view>
					</view>
					<view class="info_right">
						<view class="info_data">
							<view class="data_name">{{$t('list.dprice')}}：</view>
							<view class="data_num time_box">{{item.qpPrice}}</view>
						</view>
						<view class="info_data">
							<view class="data_name">{{$t('list.winning')}}：</view>
							<view class="data_num">{{item.buyuser}}</view>
						</view>
						<view class="info_data">
							<view class="data_name">{{$t('list.cjprice')}}：</view>
							<view class="data_num" v-if="item.paymoney>0">{{item.paymoney}}</view>
							<view class="data_num" v-if="item.paymoney==0">--</view>
						</view>
						<view class="info_data">
							<view class="data_name">{{$t('list.end')}}：</view>
							<view class="data_num time_box" style='font-size: 20rpx;'>{{item.endtime}}</view>
						</view>
					</view>
				</view>
				<!-- 加载更多动画 -->

<template>
	<view class="app">
		<view class="app_bg">
			<!-- 顶部 -->
			<topnav :title="$t('zhiya.record')" urlline="zhiya"></topnav>
			<view class="line"></view>
			<!-- 出售记录 -->
			<scroll-view class="cs_list" scroll-y="true" @scrolltolower="dddd()">
				<view class="list_info" v-for="(item,i) in mmlist">
					<view class="card_content">
						<view class="card_box" :style="'background: url(../../static/h/cards_bg/'+item.bg+'.png);background-size: 150rpx 235rpx;'">
							<view class="card_info">
								<view class="super_img" v-if="item.enName=='LOONG'"></view>
								<view class="card_name" v-if="item.enName!='RAT'&&item.enName!='DOG'&&item.enName!='OX'&&item.enName!='GOAT'&&item.enName!='PIG'&&item.enName!='LEO'">{{item.enName}}</view>
								<view class="card_rat" v-if="item.enName=='RAT'||item.enName=='DOG'||item.enName=='OX'||item.enName=='GOAT'||item.enName=='PIG'||item.enName=='LEO'">{{item.enName}}</view>
						...
						<!-- 状态 -->
						<view class="list_type" v-if="item.status==1">{{$t('zhiya.ing')}}</view>
						<view class="list_end" v-if="item.status==2">{{$t('zhiya.end')}}</view>
					</view>
					<view class="info_right">
						<view class="info_data">
							<view class="data_name">{{$t('zhiya.dayincome')}}：</view>
							<view class="data_num">{{item.dprofit}} LOO</view>
						</view>
						<view class="info_data">
							<view class="data_name">{{$t('zhiya.ljincome')}}：</view>
							<view class="data_num">{{item.profit}}LOO</view>
						</view>
						<view class="info_data">
							<view class="data_name">{{$t('zhiya.start')}}：</view>
							<view class="data_num time_box">{{item.addtime}}</view>
						</view>
						<view class="info_data">
							<view class="data_name">{{$t('zhiya.endtime')}}：</view>
							<view class="data_num time_box">{{item.cancletime}}</view>
						</view>
					</view>
				</view>
				<!-- 加载更多动画 -->

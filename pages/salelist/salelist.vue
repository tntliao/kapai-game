<template>
	<view class="app">
		<view class="app_bg">
			<!-- 顶部 -->
			<topnav :title="$t('list.saleslist')" urlline="mysale"></topnav>
			<view class="line"></view>
			<!-- 出售记录 -->
			<scroll-view class="cs_list" scroll-y="true" @scrolltolower="dddd()">
				<view class="list_info" v-for="(item,i) in salelist">
					<view class="card_content">
						...
						<view class="list_type" v-if="item.status==0">{{$t('list.listing')}}</view>
						<view class="list_type" v-if="item.status==3">{{$t('list.sold')}}</view>
					</view>
					<view class="info_right">
						<view class="info_data">
							<view class="data_name">{{$t('list.tradeprice')}}：</view>
							<view class="data_num">{{item.price}}</view>
						</view>
						<view class="info_data">
							<view class="data_name">{{$t('list.buyplayer')}}：</view>
							<view class="data_num" v-if="item.status!=3">--</view>
							<view class="data_num" v-if="item.status==3">{{item.buyacc}}</view>
						</view>
						<view class="info_data">
							<view class="data_name">{{$t('list.saletime')}}：</view>
